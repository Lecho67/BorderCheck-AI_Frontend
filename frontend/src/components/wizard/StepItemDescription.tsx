...
export function StepItemDescription({ value, onChange, onNext, onBack }: StepItemDescriptionProps) {
  const agregarChip = (chip: string) => onChange(value ? `${value}, ${chip}` : chip);

  return (
    <div>
      <p className="text-xs font-semibold text-brand-blue uppercase tracking-wide mb-2">Paso 2 de 3</p>
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        {chipsSugeridos.map((chip) => (
          <Chip key={chip} label={chip} onClick={() => agregarChip(chip)} />
        ))}
      </div>

      <div className="flex gap-3 mt-6">
        <Button variant="secondary" onClick={onBack}>← Atrás</Button>
        <Button disabled={!value.trim()} onClick={onNext}>Continuar →</Button>
      </div>
    </div>
  );
}

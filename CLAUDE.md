...
- `e2e/authenticated/rbac.spec.ts` (9): guardas de `ProtectedRoute` — cada rol fuera de su `allowedRoles` cae en su `RUTA_POR_DEFECTO`, no en `/login`. `e2e/authenticated/vistas-cliente.spec.ts` (5): carga con sesión de `/dashboard`, `/historial`, `/perfil`, `/consulta/nueva` (el `<form>`), `/casillero` (KYC aprobado pasa `RequireCompliance`).
- **Solo lectura.** Los flujos con escritura (wizard→veredicto→historial inserta en `customs_queries`; KYC→aprobación→casillero) faltan — necesitan setup/teardown de filas descartables por corrida.

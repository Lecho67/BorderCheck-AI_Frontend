...
- Las tres: página de 8, empty state propio "Ningún … coincide con los filtros", botón "Limpiar filtros" cuando hay alguno activo. Patrón visual de la barra de filtros: `flex flex-wrap items-end gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4`.
- Tests: `usePagination.test.ts`, `Pagination.test.tsx`, ampliados `AgentKycPanel.test.tsx`, `AgentDocumentsPanel.test.tsx` y `AgentPanel.test.tsx` (todos con un caso "pagina cuando hay más de 8"). **137 tests en 27 archivos.**
